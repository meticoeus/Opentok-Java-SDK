...
import com.opentok.util.HttpClientAsync;
import com.opentok.util.HttpClientAsync.ProxyAuthScheme;
 ...
 */
public class OpenTokAsync {

    private int apiKey;
    private String apiSecret;
    protected HttpClientAsync client;
     ...
     */
    public OpenTokAsync(int apiKey, String apiSecret) {
        this.apiKey = apiKey;
        this.apiSecret = apiSecret.trim();
        this.client = new HttpClientAsync.Builder(apiKey, apiSecret).build();
    }

    private OpenTokAsync(int apiKey, String apiSecret, HttpClientAsync httpClient) {
        this.apiKey = apiKey;
        this.apiSecret = apiSecret.trim();
     ...
     * <p>
     * Calling this method results in an {@link OpenTokException} in
     ...
     */
    public CompletableFuture<Session> createSession(SessionProperties properties) {
        ...
        final CompletableFuture<String> future = this.client.createSession(params);
        ...
        return future.thenApply(response -> {
            try {
                ...
                if (sessions.length != 1) {
                    throw new CompletionException(new OpenTokException(String.format("Unexpected number of sessions created %d", sessions.length)));
                }
                return new Session(sessions[0].getId(), apiKey, apiSecret, _properties);
            } catch (IOException e) {
                throw new CompletionException(new OpenTokException("Cannot create session. Could not read the response: " + response));
            }
        });
    }
     ...
     */
    public CompletableFuture<Archive> getArchive(String archiveId) {
        return this.client.getArchive(archiveId)
                .thenApply(archive -> {
                    try {
                        return archiveReader.readValue(archive);
                    } catch (Exception e) {
                        throw new CompletionException(new RequestException("Exception mapping json: " + e.getMessage()));
                    }
                });
    }
     ...
     */
    public CompletableFuture<ArchiveList> listArchives() {
     ...
     * @return A List of {@link Archive} objects.
     */
    public CompletableFuture<ArchiveList> listArchives(int offset, int count) {
        return this.client.getArchives(offset, count)
                .thenApply(archives -> {
                    try {
     ...
     * @return A List of {@link Archive} objects.
     */
    public CompletableFuture<ArchiveList> listArchives(String sessionId) {
        return this.client.getArchives(sessionId)
                .thenApply(archives -> {
                    try {
                        return archiveListReader.readValue(archives);
                    } catch (Exception e) {
     ...
     */
    public CompletableFuture<Archive> startArchive(String sessionId, ArchiveProperties properties) {
        if (sessionId == null || "".equals(sessionId)) {
            CompletableFuture<Archive> future = new CompletableFuture<>();
            future.completeExceptionally(new InvalidArgumentException("Session not valid"));
            return future;
        }
        // TODO: do validation on sessionId and name
        return this.client.startArchive(sessionId, properties)
                .thenApply(archive -> {
                    try {
                        return archiveReader.readValue(archive);
                    } catch (Exception e) {
    ...
    public CompletableFuture<Archive> startArchive(String sessionId) {
        return startArchive(sessionId, new ArchiveProperties.Builder().build());
    }

    public CompletableFuture<Archive> startArchive(String sessionId, String name) {
     ...
     */
    public CompletableFuture<Archive> stopArchive(String archiveId) {
        return this.client.stopArchive(archiveId)
                .thenApply(archive -> {
                    try {
                        return archiveReader.readValue(archive);
                    } catch (Exception e) {
     ...
     */
    public CompletableFuture<Void> deleteArchive(String archiveId) {
    ...
    public static class Builder {
        ...
        public OpenTokAsync build() {
            HttpClientAsync.Builder clientBuilder = new HttpClientAsync.Builder(apiKey, apiSecret);
            ...
            return new OpenTokAsync(this.apiKey, this.apiSecret, clientBuilder.build());
        }
    }
